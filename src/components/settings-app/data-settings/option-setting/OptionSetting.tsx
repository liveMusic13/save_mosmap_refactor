...
const OptionSetting: FC<IOptionSetting> = ({ title, data, index, length }) => {
  ...
  const renderActions = (title: string): JSX.Element => {
    switch (title) {
      ...
      case 'Группы объектов':
        return (
          <>
            <ButtonOptionSettings isMouseEnter={isMouseEnter} text='Настроить' >Настроить</ButtonOptionSettings>
  ...
  return (
    <div className={styles.wrapper_option} style={style} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <p className={styles.index} style={{ minWidth: widthConfig[title]['#'] }}>{index}</p>
      <p className={styles.name} style={{ minWidth: widthConfig[title]['name'] }}>{data.name}</p>
      {isIFields(data) ? <p className={styles.type_name} style={{ minWidth: widthConfig[title]['type_name'] }}>{data.type_name}</p> : ''}
      {isCheckbox && fieldsToCheck.map((field, idx) => (
        (data as any)[field] !== undefined && (
          <div key={idx} className={styles.block__checkbox} style={{ minWidth: widthConfig[title][field] }}>
            <input
              type="checkbox"
      ...
      <div className={styles.block__action} style={{ minWidth: widthConfig[title]['Действие'] }}>
        {renderActions(title)}
      </div>
    </div>
  );
