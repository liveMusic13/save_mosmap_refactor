  ...
  return (
    <div className={styles.wrapper_settings}>
      ...
      <div className={styles.block__content}>
        <div className={styles.block__columnTitle} style={{ gridTemplateColumns: `repeat(${length}, 1fr)` }}>
          {arrColumn.map((titleColumn) => {
            const columnWidth = widthConfig[title][titleColumn.name] || 'auto';
            ...
            return shouldDisplay ? (
              <h3 key={titleColumn.id} className={styles.column__title} style={{ minWidth: columnWidth, textAlign: titleColumn.name === '#' ? 'center' : 'start' }}>
                {titleColumn.name}
              </h3>
            ) : null;
          })}
        ...
        {
          title !== 'Карта' && ( <AddData title={title} /> )
        }
      </div>
    </div>
  )
