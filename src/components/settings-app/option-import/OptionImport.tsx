...
const OptionImport: FC = () => {
  ...
  const { isViewPopupImportDone, isImportSettingsData } = useSelector((state: RootState) => state.viewSettings);
      ...
      {isViewPopupImportDone && (
        <div className={styles.block__popupCompleted}>
          ...
          <button className={`${styles.button__send} ${styles.popup}`} onClick={()=> {
            ...
            if (isImportSettingsData) dispatch(viewSettingsActions.defaultIsImportSettingsData(''))
            getObject()
            getFilters()
          }
            }>Ок</button>
